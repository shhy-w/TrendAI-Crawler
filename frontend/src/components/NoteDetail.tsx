  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <article className="detail-drawer" onMouseDown={(event) => event.stopPropagation()}>
        <header className="detail-header"><div><span className="eyebrow">{note.note_type === 'video' ? '视频笔记' : '图文笔记'} · {completenessLabel[note.completeness] ?? note.completeness}</span><h2>{note.title || '未命名笔记'}</h2></div><button className="icon-button" onClick={onClose} title="关闭" aria-label="关闭"><X size={19} /></button></header>
