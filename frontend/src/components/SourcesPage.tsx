  ...
  return (
    <section className="page-section">
      ...
      {sources.length === 0 ? <div className="empty-state"><Rss size={28} /><strong>还没有信源</strong><span>先添加一个关键词、博主主页或笔记链接。</span><button className="primary-button" onClick={() => setShowForm(true)}><Plus size={17} />添加信源</button></div> : (
        <div className="table-wrap source-table"><table><thead><tr><th>信源</th><th>类型</th><th>采集目标</th><th>最近结果</th><th>最后运行</th><th>状态</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{sources.map((source) => <tr key={source.id}><td><strong>{source.name}</strong>{source.last_error ? <span className="row-error">{source.last_error}</span> : null}</td><td>{sourceTypeLabel[source.source_type]}</td><td className="target-cell">{source.target}</td><td>{source.last_result_count} 条</td><td>{formatDate(source.last_run_at)}</td><td><label className="switch"><input type="checkbox" checked={source.enabled} onChange={() => void onToggle(source)} /><span /><b>{source.enabled ? '启用' : '停用'}</b></label></td><td><button className="icon-button danger" title="删除信源" aria-label={`删除 ${source.name}`} onClick={() => void onDelete(source)}><Trash2 size={17} /></button></td></tr>)}</tbody></table></div>
      )}
      {showForm ? <div className="modal-backdrop" onMouseDown={() => setShowForm(false)}><div className="dialog" onMouseDown={(event) => event.stopPropagation()}><header><div><h2>添加信源</h2><p>选择一种采集入口</p></div><button className="icon-button" title="关闭" aria-label="关闭" onClick={() => setShowForm(false)}><X size={18} /></button></header><div className="segmented source-type-tabs">{(['keyword', 'profile', 'note'] as SourceType[]).map((type) => <button key={type} className={sourceType === type ? 'active' : ''} onClick={() => { setSourceType(type); setTarget(''); }}>{sourceTypeLabel[type]}</button>)}</div><label>信源名称<input value={name} onChange={(event) => setName(event.target.value)} placeholder={sourceType === 'keyword' ? '例如：AI 效率工具' : '便于识别的名称'} /></label><label>{sourceType === 'keyword' ? '搜索关键词' : sourceType === 'profile' ? '博主主页链接' : '笔记链接'}<input value={target} onChange={(event) => setTarget(event.target.value)} placeholder={sourceType === 'keyword' ? 'AI 效率工具' : 'https://www.xiaohongshu.com/...'} /></label><footer><button className="secondary-button" onClick={() => setShowForm(false)}>取消</button><button className="primary-button" disabled={saving || !name.trim() || !target.trim()} onClick={() => void submit()}>{saving ? '保存中...' : '保存信源'}</button></footer></div></div> : null}
    </section>
  );
}
