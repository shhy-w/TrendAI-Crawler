...
import { formatDate, sourceTypeLabel, statusLabel } from '../lib/format';
      ...
      <div className="job-stack">
        {jobs.map((job) => {
          ...
          return <article className="job-record" key={job.id}>
            <header><div className="job-title"><StatusIcon status={job.status} /><div><strong>任务 #{job.id}</strong><span>{formatDate(job.created_at)} · 每个信源最多 {job.max_notes_per_source} 条</span></div></div><span className={`status status-${job.status}`}>{statusLabel[job.status] ?? job.status}</span></header>
